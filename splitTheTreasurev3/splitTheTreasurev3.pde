...
void trvrse(IntList row) {
  ...
  int pttrn = row.get(iA) + row.get(iB);
  // Frst chck there's initlly engh space.
  spaceRght = chckSpace(iB, prtnrs, noElmtns);
  while (spaceRght) {
    println(iB + " " + pttrn);  // debug - delete
    // Calculate
    //println(row.get(iA) + " " + row.get(iB));  // debug - delete
    if (pttrn == targetVal) {
      //IntList spoil = new IntList();
      // pass to trgt.
      // Move onto nxt elmnt.
      ...
      println(iB + " " + pttrn);  // debug - delete
      int newB = iB++;
      boolean newChckr = chckSpace(newB, prtnrs, noElmtns);
      if (newChckr) {
        println(iB + " " + pttrn);  // debug - delete
        pttrn = pttrn + row.get(2);
        iB = newB;
        println(iB + " " + pttrn);  // debug - delete
      } else {
        spaceRght = false;
...
//--------------------------------------------------------------------------------//
boolean chckSpace(int indxB, int prtnrs, int noElmnts) {
  boolean spaceRght;

  if (indxB + prtnrs <= noElmnts) {
    spaceRght = true;
  } else {
    spaceRght = false;
  }
